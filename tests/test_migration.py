...
def test_validate_standard_schema_fails_on_missing_root_key():
    ...
    with pytest.raises(ValueError, match="Missing required key: exam_name"):
        validate_standard_schema(invalid_data)

def test_validate_standard_schema_fails_on_non_list_questions():
    invalid_data = {"exam_name": "JNU", "exam_year": 2020, "exam_title": "Test", "duration": 120, "questions": "not a list"}
    with pytest.raises(ValueError, match="questions must be a list"):
        validate_standard_schema(invalid_data)

def test_validate_standard_schema_fails_on_non_dict_question():
    invalid_data = {"exam_name": "JNU", "exam_year": 2020, "exam_title": "Test", "duration": 120, "questions": ["not a dict"]}
    with pytest.raises(ValueError, match="Question at index 0 must be a dict"):
        validate_standard_schema(invalid_data)
...
def test_validate_standard_schema_fails_on_missing_question_key():
    ...
    with pytest.raises(ValueError, match="Question at index 0 missing key: question_number"):
        validate_standard_schema(invalid_data)
